...
export default function HomePage() {
  ...
  const activeUser = CHAT_USERS.find((user) => user.id === activeUserId) ?? CHAT_USERS[0];
  ...
  async function handleDelete(messageId, scope) {
    try {
      ...
      const response = await fetch(`/api/messages/${messageId}?${search.toString()}`, {
        method: "DELETE"
      });
      const data = await response.json();
      ...
      <section className="hero">
        <div>
          ...
          <p className="hero-copy">
            A polished chat experience with persistent history, delete-for-me, delete-for-everyone,
            pinned updates, and lightweight live refresh.
          </p>
        </div>
        ...
        <div className="presence-panel">
          ...
          <p className="presence-note">
            Switch profiles to simulate another user and verify per-user delete behavior.
          </p>
        </div>
      </section>
        ...
        <section className="chat-panel">
          ...
          <div className="messages-area">
            {isLoading ? (
              ...
              <div className="status-card">No messages yet. Start the conversation below.</div>
            ) : (
              messages.map((message) => {
                ...
                return (
                  <article
                    key={message.id}
                    className={isOwn ? "message-card own-message" : "message-card"}
                  >
                    <div className="message-topline">
                      <div className="message-author">
                        <strong>{message.senderName}</strong>
                        {message.isPinned ? <span className="tag">Pinned</span> : null}
                        {message.isDeletedForEveryone ? <span className="tag muted">Deleted</span> : null}
                      </div>
                      <time>{formatTimestamp(message.createdAt)}</time>
                    </div>

                    <p className={message.isDeletedForEveryone ? "message-copy deleted-copy" : "message-copy"}>
                      {message.content}
                    </p>
                    ...
                    <div className="message-actions">
                      ...
                      <button
                        ...
                        onClick={() => handleDelete(message.id, "everyone")}
          ...
          <form className="composer" onSubmit={handleSendMessage}>
            ...
            <div className="composer-footer">
              ...
              {error ? <span className="error-text">{error}</span> : <span>Changes are saved automatically.</span>}
            </div>
          </form>
        </section>
      </section>
